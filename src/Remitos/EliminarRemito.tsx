...
const ListarRemitos = () => {
  ...
  const remitosFiltrados = fechaFiltro
    ? remitos.filter((remito) => new Date(remito.fecha).toLocaleDateString() === new Date(fechaFiltro).toLocaleDateString()): remitos;
  ...
  const eliminarRemito = async () => {
    try {
      ...
      setRemitos(remitos.filter((remito) => remito._id !== remitoSeleccionado._id));
      setModalOpen(false);
    } catch (err) {
        ...
        <tbody>
          {remitosFiltrados.map((remito) => (
            <tr key={remito._id}>
              ...
              <td>{remito.estado === "EN_ESPERA"? "En espera": remito.estado === "FACTURADO"? "Facturado": remito.estado}
              </td>
              <td>
                <button
                  className="btn-eliminar"
                  onClick={() => confirmarEliminar(remito)}>Eliminar
                </button>
              </td>
            </tr>
          ))}
